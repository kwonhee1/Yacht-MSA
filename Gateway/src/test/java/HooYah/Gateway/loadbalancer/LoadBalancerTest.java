...
class LoadBalancerTest {
    ...
    private LoadBalancerController loadBalancer;

    @BeforeEach
    void setUp() {
        server = new Server("server1", Protocol.http, new Host("localhost"), 10);

        HooYah.Gateway.loadbalancer.domain.module.Module partModule = new HooYah.Gateway.loadbalancer.domain.module.Module(
                List.of(new Uri("/part/"), new Uri("/repair/")),
                List.of(Service.running("part", server, new Port(8081))),
                new ArrayList<>()
        );

        HooYah.Gateway.loadbalancer.domain.module.Module calendarModule = new Module(
        ...
        loadBalancer = new LoadBalancerController(List.of(server), modules);
    }
