...
public class RepairServiceTest {
    ...
    public void addRepairTest() {
        ...
        Mockito.when(mockWebClient.webClient(Mockito.anyString(), Mockito.eq(HttpMethod.GET), Mockito.isNull()))
                .thenReturn(Map.of("yacht", "yacht"));
        Mockito.when(mockWebClient.webClient(Mockito.anyString(), Mockito.eq(HttpMethod.POST), Mockito.isNotNull()))
                .thenReturn(Map.of("message", "success"));

        repairService.addRepair(part.getId(), "content", OffsetDateTime.now(), userId);

        Mockito.verify(mockWebClient, Mockito.times(1)).webClient(Mockito.anyString(), Mockito.eq(HttpMethod.GET), Mockito.isNull());
        Mockito.verify(mockWebClient, Mockito.times(1)).webClient(Mockito.anyString(), Mockito.eq(HttpMethod.POST), Mockito.isNotNull());
    }
