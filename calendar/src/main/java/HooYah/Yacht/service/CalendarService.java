...
public class CalendarService {
    ...
    public CalendarInfo createCalendarByUser(CalendarCreateRequest dto, Long userId) {
        ...
        List referenceUserInfoList = askService.getUserInfoList(dto.getUserList()); // error 가 발생하기는 합니까?
        ...
        // calendar 참조인 처리
        calendarUserService.addUser(newCalendar, dto.getUserList());

        checkCompleted(newCalendar, dto.getCompleted(), userId);
...
//        }
    ...
    public List<CalendarInfo> getCalendars(Long userId) {
        ...
        return CalendarInfo.of(calendars, partInfoList, yachtInfoList, userInfoList);
    }

    @Transactional
    public Calendar updateCalendar(Long id, CalendarUpdateRequest request, Long userId) {
        Calendar calendar = calendarRepository.findById(id)
                .orElseThrow(()->new CustomException(ErrorCode.NOT_FOUND));
        ...
        checkCompleted(calendar, request.getCompleted(), userId);
    ...
    private void checkCompleted(Calendar calendar, boolean requestCompleted, Long userId) {
        if(calendar.isCompleted() || !requestCompleted)
            return;

        calendar.setCompletedTrue();

        // repair domain에게 무한 web client 시도하기
        String uri = String.format(gatewayURL + createRepairURI + "?userId=%d", userId);
        try {
            webClient.webClient(uri, HttpMethod.POST, Map.of("date", OffsetDateTime.now(), "content", "autoGenerated"));
